...
class OTPRegisterConfirm extends StatefulWidget {
  const OTPRegisterConfirm({Key? key, required this.timer}) : super(key: key);
  final Timer timer;
      ...
      child: Column(
        children: [
          const Text(
            ...
            style: TextStyle(
                fontWeight: FontWeight.bold, fontSize: 20, color: color777777),
          ),
          const SizedBox(
            height: 20,
          ),
          ...
          Text(
              "${timeFormat.format(context.watch<RegisterInfoProvider>().minutes)} : ${timeFormat.format(context.watch<RegisterInfoProvider>().seconds)}"),
          const SizedBox(
            height: 20,
          ),
        ],
